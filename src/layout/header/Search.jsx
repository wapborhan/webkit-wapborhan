...
// search data
import { blocksSearchData, componentSearchData } from "@/utils/SearchData";
...
const Search = ({ isSearchOpen }) => {
  const [filteredComponentData, setFilteredComponentData] =
    useState(componentSearchData);
  const [filteredBlocksData, setFilteredBlocksData] =
    useState(blocksSearchData);
        ...
        >
          <div className="relative bg-white pb-[10px]">
            ...
            <input
              ...
              type="text"
            />
            <span className="bg-gray-200 rounded-md px-3.5 py-1.5 text-gray-500 absolute top-[25px] transform -translate-y-1/2 right-1.5">
              Esc
            </span>
          </div>
          ...
          <div className="h-[65vh]">
            {filteredComponentData?.length > 0 && (
              <div className="mt-5">
                <h3 className="text-gray-600 font-bold pb-[10px]">
                  Components
                </h3>

                <div
                  className={`${
                    filteredBlocksData?.length <= 0 ? "h-full" : "max-h-[25vh]"
                ...
                >
                  {filteredComponentData?.map((component, index) => (
                    ...
                    >
                      ...
                      <p className="text-[1rem] font-[500] text-gray-500 capitalize">
                        {component.title}
                      </p>
                    </Link>
                  ))}
            ...
            {filteredBlocksData?.length > 0 && (
              <div className="mt-5">
                ...
                <div
                  className="flex flex-col max-h-[20vh] 1024px:max-h-[25vh] overflow-y-auto"
                  style={{ scrollbarWidth: "none" }}
                >
                  {filteredBlocksData?.map((block, index) => (
                    ...
                    >
                      ...
                      <p className="text-[1rem] font-[500] text-gray-600 capitalize">
                        {block.title}
                      </p>
                    </Link>
                  ))}
